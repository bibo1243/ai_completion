...
export const SmartDateInput = ({ label, value, onChange, theme, tasks, innerRef, colorClass }: any) => {
  ...
  return (
    <div className="relative" ref={containerRef} onBlur={handleBlur}>
      ...
      >
        <Calendar size={13} />
        <span>{value ? getRelativeDateString(value, false) : label}</span>
      ...
      {isOpen && (
        <div className="absolute top-full mt-2 left-0 bg-white rounded-xl shadow-2xl border border-gray-100 p-4 z-50 w-72 max-w-[90vw] animate-in fade-in zoom-in duration-100">
          <div className="mb-3 relative">
            <input autoFocus type="text" placeholder="輸入: 5, 明天, 下週五..." className="w-full text-xs border border-gray-200 rounded p-2 focus:ring-2 focus:ring-indigo-100 outline-none pr-8" value={text} onChange={handleInputChange} onKeyDown={handleKeyDown} />
            {highlightedDate && <div className="absolute right-2 top-2 text-[10px] text-indigo-500 font-bold bg-indigo-50 px-1 rounded animate-pulse">識別: {formatDate(highlightedDate.toISOString())}</div>}
          </div>
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="font-bold text-sm text-gray-800">{currentDate.toLocaleString('default', { month: 'long', year: 'numeric' })}</span>
          ...
          <div className="border-t border-gray-100 pt-3 mt-3">
            <div className="text-[10px] font-bold text-gray-400 mb-2 flex justify-between items-center">
              <span>{previewDate ? `${previewDate.getMonth() + 1}/${previewDate.getDate()} 行程` : '行程'}</span>
              <span className="bg-gray-100 px-1.5 rounded text-gray-500">{previewTasks.length}</span>
            </div>
            <div className="max-h-24 overflow-y-auto space-y-1">
              {previewTasks.length > 0 ? previewTasks.map((t: any) => (<div key={t.id} className="text-[10px] bg-gray-50 p-1 rounded truncate border-l-2 border-indigo-400 pl-2 text-gray-600">{t.title}</div>)) : <div className="text-[10px] text-gray-300 italic text-center py-2">無安排任務</div>}
            </div>
          </div>
          <button type="button" onClick={confirmSelection} className="w-full mt-3 bg-indigo-600 text-white text-xs font-bold py-1.5 rounded hover:bg-indigo-700 transition-colors">確認日期</button>
        </div>
      )}
    </div>
  );
